...
import {
   ...
   * @param {NamedArg<T, N>} argCreator - The original argument creator function
   * @param {(value: U) => T} transformer - Function that transforms the input value to the required type
   * @returns {NamedArg<U, N>} A new argument creator that accepts U and applies the transformation
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  export function transformArg<T, U, N extends string>(
    argCreator: NamedArg<T, N>,
    transformer: (value: U) => T
  ): NamedArg<U, N> {
    return (value: U): BrandedArg<U, N> => {
      ...
      // Cast the branded argument to the expected type
      return brandedArg as unknown as BrandedArg<U, N>;
    };
  }
